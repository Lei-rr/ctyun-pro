      ...
      <div class="space-y-2">
        ...
        >
          ...
          <div class="text-[11px] text-muted-foreground flex items-center justify-between font-mono pt-1">
            <span>{{ (task.type === 'hang' || task.name.includes('使用') || task.name.includes('时长') || task.name.includes('体验')) ? `已累计保活: ${Math.floor(task.currentProgress / 60)} / ${Math.floor(task.totalProgress / 60)} 分钟 (${task.currentProgress}/${task.totalProgress}秒)` : `完成度: ${task.currentProgress} / ${task.totalProgress}` }}</span>
            <div class="flex items-center gap-2">
              <!-- 1. 挂机与登录类任务：提示保活自动达成 -->
              <template v-if="task.type === 'hang' || task.name.includes('使用') || task.name.includes('时长') || task.name.includes('体验') || task.name.includes('登录')">
                <span v-if="task.isCompleted" class="text-emerald-500 font-sans">
                  已达标
                </span>
