...
- 🖥️ **一等公民 Desktops 直连系统**：提供 `/desktop/:id` 顶级标准直连推流通道，点击直接在浏览器原生标签页中秒开操作云电脑；集成 Keyboard Lock API 锁定独占快捷键（Alt+Tab、Win 键、Ctrl+W），带来沉浸式操作体验；
- 🔄 **凭证无感静默轮转**：内置自动化凭证巡检与免密无感换票守护机制，长期运行无需手动重新登录或输入验证码，维持账号会话长久保鲜；
...
> 💡 **官方镜像地址**：
> - **GitHub 官方源（海外/默认）**：`ghcr.io/lei-rr/ctyun-pro:latest` 或 `ghcr.io/lei-rr/ctyun-pro:v2.1.0`
> - **阿里云高速源（国内推荐 ⚡）**：`crpi-kbafcu5p49r7b1k1.cn-hangzhou.personal.cr.aliyuncs.com/lei-rr/ctyun-pro:latest` 或 `crpi-kbafcu5p49r7b1k1.cn-hangzhou.personal.cr.aliyuncs.com/lei-rr/ctyun-pro:v2.1.0`

---

## 🌟 核心亮点

- 🖥️ **顶级 RESTful 架构**：全栈统一为 `desktop` 顶级标准体系，提供干净规范的 `/desktop/:id` 直连与 `/api/desktops/*` 管理接口。
