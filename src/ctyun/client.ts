...
export class CtYunClient {
  ...
  private async fetchDesktopsByIds(objIds: string[]): Promise<Desktop[]> {
    ...
    for (let i = 0; i < objIds.length; i += 30) {
      ...
      try {
        const res = await safeFetch(`${CtYunClient.BASE_URL}/api/desktop/client/listDesktopByIds`, {
          method: 'POST',
          headers: {
            ...this.getHeaders(),
            'Content-Type': 'application/json',
          },
          body: JSON.stringify({ objIds: batch }),
        });
  ...
  public async connectDesktop(desktopIdOrObj: string | Desktop, objType = 0, forceFresh = false): Promise<DesktopInfo> {
    ...
    const buildApiUrl = (path: string, base?: string): string => `${base || CtYunClient.BASE_URL}${path}`;
    ...
    if (!forceFresh && targetObjType === 0) {
      try {
        ...
        const sRes = await safeFetch(buildApiUrl(statusPath), { headers: this.getHeaders() });
    ...
    for (const base of apiBases) {
      try {
        const res = await safeFetch(buildApiUrl(connectPath, base), {
          method: 'POST',
          headers: {
            ...this.getHeaders(),
            'Content-Type': 'application/x-www-form-urlencoded',
  ...
  public async listOrderInstStatistics(
    ...
    const res = await safeFetch(`${CtYunClient.SELFORDER_URL}/api/desktop-admin/order/mgr/listOrderInstStatisticsV2`, {
      method: 'POST',
      headers: {
        ...this.getHeaders(),
        'Content-Type': 'application/json',
